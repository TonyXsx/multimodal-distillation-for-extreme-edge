"""
Cheap greedy KD hyperparameter sweep. Small student, 100% data, one seed.

Doing it on the small student because that's where KD actually has signal -
on strong+100% the effect is about zero and there'd be nothing to optimise.
Selection on val, test untouched.

  1. CE-only anchor
  2. logit-KD: T in {2,4,8} at lam_logit=0.5, pick the best T, then sweep
     lam_logit in {0.1, 1.0} at that T
  3. feature-KD: lam_feature in {0.3, 1.0, 3.0}
  4. full-KD with the best of each, plus one with a smaller lam_logit

T=16 and lam_logit=3.0 are boundary probes. They're recorded next to their
group so the csv reads in sweep order, but left out of the argmax so selection
stays on the original grid.

writes results.csv and sweep.png to outputs/student/hparam_sweep/.
"""
...
OUT = OUTPUTS_ROOT / "fsc" / "student" / "hparam_sweep"
...
def main():
    ...
    # one step past the grid, recorded but not selected on
    record("logit_T_sweep", "T16_ll0.5", 16.0, 0.5, 0.0,
           run(train_data, val_data, t=16.0, lam_logit=0.5, lam_feature=0.0))

    # 2b. lam_logit sweep at the best T (0.5 already done in 2a)
    ...
    # again just a probe, not selected on
    ...
    # 4. full KD with the best of each
    print("\n# 4. Full-KD")
